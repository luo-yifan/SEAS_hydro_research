...
## Priors for Stage Fall Discharge Coefficient (c) and Exponents (d & f) for St Clair River ##
  ...
  # SFDc = 201.03   # sk) From Dani's code. Not sure of source
  # SFDd = 1.732    # sk) From Dani's code. Not sure of source
  # SFDf = 0.428    # sk) From Dani's code. Not sure of source
  ...
  for (j in PosteriorStartMonth:PosteriorEndMonth){
    ...
    MHLogOutflow[j]  ~dnorm(LogOutflowMeanMH[j], 25) ## in future, do not fix this value. Allow precision to be calculated for c, d, f separately #11/15 changed to 25
#    LogOutflowMeanMH[j] = log(SFDc_STC) + SFDd_STC*log((miHuronBOMPost[j]+stClairBOMPost[j])/2 - 167) # no fall component
#    LogOutflowMeanMH[j] = log(SFDc_STC) + SFDd_STC*log((miHuronBOMPost[j]+stClairBOMPost[j])/2 - 167) + SFDf_STC*log(miHuronBOMPost[j]-stClairBOMPost[j])
    LogOutflowMeanMH[j] = log(SFDc_STC) + SFDd_STC*log(stClairBOMPost[j] - 167) + SFDf_STC*log(miHuronBOMPost[j]-stClairBOMPost[j])  ## Formulation consistent with Thompson et al (2020)
    ...
    ####### Add in Detroit River SFD code here. Model after MHOutflow #######
