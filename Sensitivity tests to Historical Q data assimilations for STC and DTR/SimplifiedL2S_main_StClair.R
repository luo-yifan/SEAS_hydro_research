...
library(rjags);
setwd("/Users/skatzees/Dropbox (University of Michigan)/Hydro L2SWBM GSRA/WaterBalance/SFD_Indep_Q_Est/")
...
outputname = 'Nov17_v4_SFD_StClair_Q_200k_PrecOnQis25'
...
# OutflowShapeMH          =NULL   #sk 10/14 comment for SFD
# OutflowMeanMH           =NULL   #sk 10/14 comment for SFD
# OutflowPrecisionMH      =NULL   #sk 10/14 comment for SFD
InflowShapeMH           =NULL
InflowMeanMH            =NULL
...
# InflowShapeLkStCl       =NULL   #sk 10/14 comment for SFD
# InflowMeanLkStCl        =NULL   #sk 10/14 comment for SFD
# InflowPrecisionLkStCl   =NULL   #sk 10/14 comment for SFD
###### Erie ######
PrecipShapeEri          =NULL
PrecipRateEri           =NULL
...
for (i in 1:12){
  ...
  ## Michigan-Huron ##
#sk  OutflowDatMH          =PriorMHOutData[as.numeric(format(as.Date(PriorMHOutData[,6]),"%m"))==i,Outflow.Column.Number]
#sk  OutflowDatMH          =as.numeric(do.call(c,OutflowDatMH))
#sk  OutflowMeanMH[i]      =mean(OutflowDatMH,na.rm = TRUE)
#sk  OutflowPrecisionMH[i] =(1/var(OutflowDatMH,na.rm = TRUE))
  ...
  ## LAKE St. Clair ##
#  InflowDatLkStCl         =PriorLkStClInflData[as.numeric(format(as.Date(PriorLkStClInflData[,6]),"%m"))==i,Inflow.Column.Number]  #not needed with new Lake-to-Lake routing model as prior
...
miHuronBOMPost = as.numeric(miHuronBOM[PosteriorRange,3])  #sk 10/14/21. This is needed for the new Lake-to-Lake model of channel flow
...
stClairBOMPost = as.numeric(stClairBOM[PosteriorRange,3])
...
#yLkStClInfl1  =as.numeric(stClairInflow[PosteriorRange,3]) #check if these are needed for plotter. Taken out to get lake-to-lake model code to run
#yLkStClInfl2  =as.numeric(stClairInflow[PosteriorRange,4]) #check if these are needed for plotter. Taken out to get lake-to-lake model code to run
###### Erie ######
...
inputDataCoreJAGS=list(
  "stClairBOMPost"      =stClairBOMPost,
  "miHuronBOMPost"      =miHuronBOMPost,
...
#sk  "OutflowMeanMH"       =OutflowMeanMH,
#sk  "OutflowPrecisionMH"  =OutflowPrecisionMH,
...
#sk  "yMHOutflow1"         =yMHOut1,
#sk  "yMHOutflow2"         =yMHOut2,
#sk  "yMHOutflow3"         =yMHOut3,
...
jMod = jags.model(file = "SimplifiedL2S_jags_StClair.jags.r", data = inputDataCoreJAGS, n.chains = 3) 
...
# source('SimplifiedL2S_plotter_5y_sk_v3.R')
# cat(date(),'DONE!\n')
