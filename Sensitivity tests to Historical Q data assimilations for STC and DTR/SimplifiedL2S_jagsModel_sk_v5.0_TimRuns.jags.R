...
## Priors for Stage Fall Discharge Coefficient (c) and Exponents (d & f) for St Clair River ##
	SFDc_STC ~ dnorm(250,0.0004)T(1,1000) ## Stdev is 50 (prec 0.0004) to "set" range from 0-500, ~99.99994% of the time ## Added T to force a range only between 1 and 1000
	SFDd_STC ~ dnorm(1.6, 0.25) # try dgamma(shape=1.6,rate=2)
	SFDf_STC ~ dnorm(0.5, 0.5)T(0,3)    ## Added T to force a range only between 0 and 3 ~ dnorm(0.5, 1)T(0,3)
	# SFDc_STC ~ dnorm(250,0.0004)T(1,1000) ## Stdev is 50 (prec 0.0004) to "set" range from 0-500, ~99.99994% of the time ## Added T to force a range only between 1 and 1000
	# SFDd_STC ~ dnorm(1.6, 0.25) # try dgamma(shape=1.6,rate=2)
	# SFDf_STC ~ dnorm(0.5, 0.5)T(0,3)    ## Added T to force a range only between 0 and 3 ~ dnorm(0.5, 1)T(0,3)
	
## Priors for Stage Fall Discharge Coefficient (c) and Exponents (d & f) for Detroit River ##

	SFDc_DTR ~ dnorm(55, 0.004)T(1,500) ## Stdev is 16 (prec 0.0039) 
	SFDd_DTR ~ dnorm(2.1, 0.25) # try dgamma(shape=1.6,rate=2)
	SFDf_DTR ~ dnorm(0.4, 0.5)T(0,3) 
	# SFDc_DTR ~ dnorm(55, 0.004)T(1,500) ## Stdev is 16 (prec 0.0039) 
	# SFDd_DTR ~ dnorm(2.1, 0.25) # try dgamma(shape=1.6,rate=2)
	# SFDf_DTR ~ dnorm(0.4, 0.5)T(0,3) 
  ...
  for (j in PosteriorStartMonth:PosteriorEndMonth){
    ...
    ####### Add in St. Marys River SFD code here. Model after MHOutflow #######
    ...
    # SFD code for St Clair River Flow below   # added SK and DG 10/18/21
    MHOutflow[j]     =exp(MHLogOutflow[j])
    MHLogOutflow[j]  ~dnorm(LogOutflowMeanMH[j], 100) ## was 0.01. In future, do not fix this value. Allow precision to be calculated for c, d, f separately
#    LogOutflowMeanMH[j] = log(SFDc_STC) + SFDd_STC*log((miHuronBOMPost[j]+stClairBOMPost[j])/2 - 167) #+ SFDf_STC*log(miHuronBOMPost[j]-stClairBOMPost[j])
    # LogOutflowMeanMH[j] = log(SFDc_STC) + SFDd_STC*log((miHuronBOMPost[j]+stClairBOMPost[j])/2 - 167) + SFDf_STC*log(miHuronBOMPost[j]-stClairBOMPost[j])
    LogOutflowMeanMH[j] = log(SFDc_STC) + SFDd_STC*log(miHuronBOMPost[j] - 167) + SFDf_STC*log(miHuronBOMPost[j]-stClairBOMPost[j])
    
    ###### LAKE St. Clair ######
    ## Uses NBS instead of P, E, R - easier calculations
    LkStClNBS[j]        ~dnorm(NBSMeanLkStCl[m[j]],NBSPrecisionLkStCl[m[j]])
# SFD code for St Clair River Flow below   # added SK 10/28/21
    LkStClOutflow[j]     =exp(LkStClLogOutflow[j])
    LkStClLogOutflow[j]  ~dnorm(LogOutflowMeanLkStCl[j], 100) ##  was 0.01. In future, do not fix this value. Allow precision to be calculated for c, d, f separately
    # LogOutflowMeanLkStCl[j] = log(SFDc_DTR) + SFDd_DTR*log((stClairBOMPost[j]+erieBOMPost[j])/2 - 167) + SFDf_DTR*log(stClairBOMPost[j]-erieBOMPost[j])
    LogOutflowMeanLkStCl[j] = log(SFDc_DTR) + SFDd_DTR*log(stClairBOMPost[j] - 164) + SFDf_DTR*log(stClairBOMPost[j]-erieBOMPost[j])
    
#SK    LkStClOutflow[j]     ~dnorm(OutflowMeanLkStCl[m[j]],OutflowPrecisionLkStCl[m[j]])
    ...
    ####### Add in Niagara River SFD code here. Model after MHOutflow #######
    ...
    #
    #  yLkStClOutflow1[j]     ~dnorm(yLkStClOutflow1Mean[j],yLkStClOutflow1Precision) ## Detroit IGS ##
    #  yLkStClOutflow1Mean[j] =LkStClOutflow[j]+yLkStClOutflow1Bias[m[j]]
    #  yLkStClOutflow2[j]     ~dnorm(yLkStClOutflow2Mean[j],yLkStClOutflow2Precision) ## Detroit SFD.ADVM ##
    # yLkStClOutflow2Mean[j] =LkStClOutflow[j]+yLkStClOutflow2Bias[m[j]]
...
#    yLkStClNBS2Bias[jp]     ~dnorm(0,0.01) 
    ...
    #  yLkStClOutflow1Bias[jp] ~dnorm(0,0.01)
    # yLkStClOutflow2Bias[jp] ~dnorm(0,0.01)
...
#  yLkStClNBS2Precision      ~dlnorm(-log(100),1)#dgamma(0.1,0.1)
  ...
  #  yLkStClOutflow1Precision  ~dlnorm(-log(10000),1)#dgamma(0.1,0.1)
  # yLkStClOutflow2Precision  ~dlnorm(-log(10000),1)#dgamma(0.1,0.1)
