model{
  ...
  SFDf_DTR ~ dnorm(0.4, 0.5)T(0,3)     ## Added T to force a range only between 0 and 3
  ...
  for (j in PosteriorStartMonth:PosteriorEndMonth){
    ...
    #LkStClOutflow[j]     ~dnorm(OutflowMeanLkStCl[m[j]],OutflowPrecisionLkStCl[m[j]]) # remove for SFD
    ####### Add in Detroit River SFD code here #######
    LkStClOutflow[j]     =exp(LkStClLogOutflow[j])
    LkStClLogOutflow[j]  ~dnorm(LogOutflowMeanLkStCl[j], 0.01) ## in future, do not fix this value. Allow precision to be calculated for c, d, f separately #11/15 changed to 25
    # LogOutflowMeanLkStCl[j] = log(SFDc_DTR) + SFDd_DTR*log((stClairBOMPost[j]+erieBOMPost[j])/2 - 164) # no fall component
    # LogOutflowMeanLkStCl[j] = log(SFDc_DTR) + SFDd_DTR*log((stClairBOMPost[j]+erieBOMPost[j])/2 - 164) + SFDf_DTR*log(stClairBOMPost[j]-erieBOMPost[j])
    LogOutflowMeanLkStCl[j] = log(SFDc_DTR + SFDd_DTR*log(erieBOMPost[j] - 164) + SFDf_DTR*log(stClairBOMPost[j]-erieBOMPost[j]))  ## Formulation consistent with Thompson et al (2020)
