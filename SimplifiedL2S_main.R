...
library(rjags);
setwd("~/WorkSpace/L2SWBM")

## Add-in by Dani ##
##### Config setup (workaround) ####
iters = as.integer(as.vector(100000))
halfIters = iters/2;
## End add-in ##
##### Set output file(s) name #####
outputname = 'June7_trial1_19502021_100k'
...
MHOutPriorRange0=which(as.numeric(format(as.Date(miHuronOutflow[,6]),"%Y"))==startPriorYear  
                       & as.numeric(format(as.Date(miHuronOutflow[,6]),"%m"))==startPriorMonth)
MHOutPriorRange=which(as.numeric(format(as.Date(miHuronOutflow[,6]),"%Y"))==endPriorYear
...
LkStClOutPriorRange0=which(as.numeric(format(as.Date(stClairOutflow[,6]),"%Y"))==startPriorYear 
                       & as.numeric(format(as.Date(stClairOutflow[,6]),"%m"))==startPriorMonth)
LkStClOutPriorRange=which(as.numeric(format(as.Date(stClairOutflow[,6]),"%Y"))==endPriorYear
...
EriOutPriorRange0=which(as.numeric(format(as.Date(erieOutflow[,5]),"%Y"))==startPriorYear  ## Change to [,4] if only one dataset used
                        & as.numeric(format(as.Date(erieOutflow[,5]),"%m"))==startPriorMonth) ## Change to [,4] if only one dataset used
EriOutPriorRange=which(as.numeric(format(as.Date(erieOutflow[,5]),"%Y"))==endPriorYear ## Change to [,4] if only one dataset used
...
startAnalysisYear = 1950;
startAnalysisMonth = 1;
endAnalysisYear = 2021;
endAnalysisMonth = 12;
dateAll = as.Date(miHuronBOM[,5]) # just using miHuronBOM b/c all csvs are same date range
